                     ...
                     LW_IN struct timespec *abstime)
{
    ...
    if (lwcondvar->waitq == LW_WAITER_ID_MAX) {
        /* First waiter */
        lwcondvar->waitq = waiter->id;
        waiter->prev = LW_WAITER_ID_MAX;
    } else {
        for (existing_waiter = lw_waiter_from_id(lwcondvar->waitq);
...
lw_condvar_signal(LW_INOUT lw_condvar_t *lwcondvar)
{
    ...
    lw_mutex2b_lock(&lwcondvar->waitq_lock);
    to_wake_up = lw_waiter_from_id(lwcondvar->waitq);
    if (to_wake_up != NULL) {
        lwcondvar->waitq = to_wake_up->next;
...
lw_condvar_broadcast(LW_INOUT lw_condvar_t *lwcondvar)
{
    ...
    lw_mutex2b_lock(&lwcondvar->waitq_lock);
    list_to_wake_up = lwcondvar->waitq;
